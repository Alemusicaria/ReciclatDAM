...
@section('content')
    <div class="container-fluid h-100 mt-5 pt-5">
        <h1>Edita el perfil de l'usuari: {{ e($user->nom) }} {{ e($user->cognoms) }}</h1>
        <form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="nom" name="nom" value="{{ e($user->nom) }}">
            </div>

            <div class="form-group">
                <label for="cognoms">Cognoms</label>
                <input type="text" class="form-control" id="cognoms" name="cognoms" value="{{ e($user->cognoms) }}">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="telefon" name="telefon" value="{{ e($user->telefon) }}">
            </div>

            <div class="form-group">
                <label for="ubicacio">Ubicació</label>
                <textarea class="form-control" id="ubicacio" name="ubicacio">{{ e($user->ubicacio) }}</textarea>
            </div>
            ...
            <div class="form-group">
                ...
                @if($user->foto_perfil)
                    @if(str_starts_with($user->foto_perfil, 'https://'))
                        <img src="{{ e($user->foto_perfil) }}" alt="Profile Photo" class="rounded-circle"
                            style="width: 30px; height: 30px; margin-right: 10px;">
                    @elseif(file_exists(public_path('storage/' . e($user->foto_perfil))))
                        <img src="{{ asset('storage/' . e($user->foto_perfil)) }}" alt="Profile Photo" class="rounded-circle"
                            style="width: 30px; height: 30px; margin-right: 10px;">
                    @else
